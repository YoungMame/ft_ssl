...
def build_file_test(test_file, key):
    return ["./ft_ssl", "3des-ecb", "-i", test_file, "-k", key], \
           ["openssl", "des-ede3-ecb", "-in", test_file, "-K", key, "-provider", "default", "-provider", "legacy"]

def build_out_file_test(test_file, test_out_file, key):
    return ["./ft_ssl", "3des-ecb", "-i", test_file, "-o", test_out_file + "_ft_ssl", "-k", key], \
           ["openssl", "des-ede3-ecb", "-in", test_file, "-out", test_out_file + "_openssl", "-K", key, "-provider", "default", "-provider", "legacy"]
def build_encode_decode_cross_test(test_file, test_out_file, test_decrypted_file, key):
    return ["./ft_ssl", "3des-ecb", "-i", test_file, "-o", test_out_file + "_ft_ssl", "-k", key], \
           ["openssl", "des-ede3-ecb", "-in", test_file, "-out", test_out_file + "_openssl", "-K", key, "-provider", "default", "-provider", "legacy"], \
           ["./ft_ssl", "3des-ecb", "-d", "-i", test_out_file + "_openssl", "-o", test_decrypted_file + "_ft_ssl", "-k", key], \
           ["openssl", "des-ede3-ecb", "-d", "-in", test_out_file + "_ft_ssl", "-out", test_decrypted_file + "_openssl", "-K", key, "-provider", "default", "-provider", "legacy"]
...
file_tests = [
    # [ "test/files/binary" ],
    [ "test/files/text", "0C871EEA3A53959353267524379562934659463564932562" ],
    [ "test/files/image.png", "0C871EEA3A53959353267524379562934659463564932562" ],
]

out_file_tests = [
    [ "test/files/text", "test/files/.out/des_ecb_test_output_text", "0C871EEA3A53959353267524379562934659463564932562" ],
    [ "test/files/binary", "test/files/.out/des_ecb_test_output_binary", "0C871EEA3A53959353267524379562934659463564932562" ],
    [ "test/files/image.png", "test/files/.out/des_ecb_test_output_image.png", "0C871EEA3A53959353267524379562934659463564932562" ],
]

encode_decode_cross_tests = [
    [ "test/files/text", "test/files/.out/text.encrypted", "test/files/.out/text.decrypted","0C871EEA3A53959353267524379562934659463564932562" ],
    [ "test/files/binary", "test/files/.out/binary.encrypted", "test/files/.out/binary.decrypted","0C871EEA3A53959353267524379562934659463564932562" ],
...
def tests():
    ...
    for test_file, test_out_file, test_decrypted_file, password, salt in encode_decode_cross_tests:
        if not run_cmd_pair_x2(build_encode_decode_cross_test_pbkdf(test_file, test_out_file, test_decrypted_file, password, salt)):
            ...
            print("Test des-ecb encode-decode cross with PBKDF passed for file:", test_file);
